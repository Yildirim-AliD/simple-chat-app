...
class LoginWindow(QtWidgets.QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Login to PyMessenger")
...
class ChatWindow(QtWidgets.QWidget):
    def __init__(self):
        ...
        self.chat_label = QtWidgets.QLabel("Chat", self)
        ...
        self.user_label = QtWidgets.QLabel("Online Users", self)
    ...
    def show_context_menu(self,pos):
        ...
        private_msg_action = menu.addAction("Send Private Message")
        view_profile_action = menu.addAction("View User Profile")
    ...
    def view_user_profile(self):
        ...
        if selected_user:
            user_profile = f'User: {selected_user.text()}\n'
            QtWidgets.QMessageBox.information(self, "User Profile", user_profile)
    ...
    def set_private_message_target(self):
        ...
        if selected_user:
            ...
            if target_user == nickname:
                QtWidgets.QMessageBox.warning(self, "Error", "You cannot send a message to yourself.")
                target_user=None
                return
            else:
                self.message_label.setText(f"You are sending a private message to {target_user}.")
    ...
    def open_emoji_picker(self):
        ...
        emoji_dialog.setWindowTitle("Emoji Picker")
    ...
    def receive_message(self,message):
        if message.startswith('USER_LIST'):
            ...
            if target_user == nickname:
                self.chat_area.append(f"Private message sent to you: {private_message}")
        else:
    ...
    def send_message(self):
        ...
        if message.strip():
            ...
            if target_user:
                full_message = f'[{timestamp}] [Private] {nickname} -> {target_user}: {message}'
                client.send(f'PRIVATE:{target_user}:{full_message}'.encode('utf-8'))
                self.chat_area.append(f'A private message has been sent to {target_user}: {message}')
            else:
